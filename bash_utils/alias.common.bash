...
## -c, to sum up count
alias wg="ag  --follow --noheading --ignore-case --hidden --path-to-agignore ~/.agignore"
...
eba() {
   if [ "$#" -eq 0 ]; then
       vim $HOME/bash_utils/alias.common.bash && sbr
   else
       vim $HOME/bash_utils/alias.$1.bash && sbr
   fi
}
md () { mkdir -p "$1"; }
mcd () { mkdir -p "$1" && cd "$1"; }
